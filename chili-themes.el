;;; chili-themes.el --- A version of Chris Kempson's "tomorrow" themes
...
;;     M-x chili-theme-day
;;     M-x chili-theme-night
;;     M-x chili-theme-blue
;;     M-x chili-theme-bright
;;     M-x chili-theme-eighties
;;
;;; Credit:
...
(defconst chili-theme-colors
  '((dark . ((background . "#181a26")
             (current-line . "#1F2232")
             (selection . "#103050")
             (foreground . "#eaeaea")
             (comment . "#969896")
...
(defmacro chili-theme--with-colors (mode &rest body)
...
`MODE' should be set to either 'day, 'night, 'eighties, 'blue or 'bright."
  `(let* ((colors (or (cdr (assoc ,mode chili-theme-colors))
...
(defmacro chili-theme--face-specs ()
  ...
  (quote
   (mapcar
    ...
    `(;; Standard font lock faces
      ...
      (font-lock-keyword-face (:foreground ,green :weight semi-bold))
      ...
      (fill-column-indicator (:foreground ,highlight))
      (success (:foreground ,green :weight bold))
      (error (:foreground ,red :weight bold))
      (warning (:foreground ,orange :weight bold))
      (tooltip (:foreground ,foreground :background ,background :height 0.8))

      ;; Emacs interface
      (cursor (:background ,red))
      (fringe (:background ,background :foreground ,comment))
      (line-number (:background ,background :foreground ,comment :italic nil :underline nil :height 0.8))
      ...
      (org-block (:extend t :background ,highlight))
      (org-block-begin-line (:extend t :background ,contrast-bg :foreground ,comment :slant italic))
      (org-block-end-line (:extend t :background ,contrast-bg :foreground ,comment :slant italic))
      ...
      ;; outline-minor-faces
      (outline-minor-0 (:weight bold :background ,highlight))
      ...
      (which-func (:foreground ,blue :background nil :weight normal))
      ...
      ;; transient
      (transient-enabled-suffix (:foreground ,background :background ,green :weight bold))
...
(provide 'chili-themes)

;; Local Variables:
;; byte-compile-warnings: (not cl-functions)
;; End:

;;; chili-themes.el ends here
